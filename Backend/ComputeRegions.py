...
def plot_voronoi_result(regions_gdf, voronoi_gdf, clipped_gdf, output_image=None):
    ...
    regions_gdf.plot(ax=ax, color="white", edgecolor="black", alpha=0.1)

    # Voronoi polygons (before clipping)
    voronoi_gdf.plot(ax=ax, facecolor="none", edgecolor="blue", linewidth=0.1, alpha=0.8)

    # Clipped Voronoi (final desired shapes)
    clipped_gdf.plot(ax=ax, cmap="tab20", alpha=0.65, edgecolor="black", linewidth=0.10)

    # Practitioner points
    pts = voronoi_gdf.copy()
    pts["geometry"] = pts.geometry.centroid
    pts.plot(ax=ax, color="red", markersize=0.01, label="Praticiens")
